...
public class MixinNpcWrapper<T extends EntityNPCInterface> extends EntityLivingWrapper<T> {
    ...
    public void syncAnimationsFor(IPlayer player, String animName) {
        NetworkWrapper.send(player.getMCEntity(), new PacketSyncAnimation(entity.getId(), animName, false, PacketSyncAnimation.TYPE_MANUAL));
    }
    @Unique
    public void syncAnimationsForAll(String animName) {
        NetworkWrapper.sendAll(new PacketSyncAnimation(entity.getId(), animName, false, PacketSyncAnimation.TYPE_MANUAL));
    }
    @Unique
    public void syncInstantAnimationsFor(IPlayer player, String animName) {
        NetworkWrapper.send(player.getMCEntity(), new PacketSyncAnimation(entity.getId(), animName, true, PacketSyncAnimation.TYPE_MANUAL));
    }
    @Unique
    public void syncInstantAnimationsForAll(String animName) {
        NetworkWrapper.sendAll(new PacketSyncAnimation(entity.getId(), animName, true, PacketSyncAnimation.TYPE_MANUAL));
    }
}