...
public abstract class MixinBlockScriptedWrapper extends BlockWrapper {
    ...
    @Unique
    public void syncAnimForPlayer(String animName, IPlayer<ServerPlayer> player) {
        NetworkWrapper.sendToPlayer(new PacketSyncTileAnimation(getMCTileEntity().getBlockPos(), animName), player.getMCEntity());
    }

    @Unique
    public void syncAnimForAll(String animName) {
        NetworkWrapper.sendToAll(new PacketSyncTileAnimation(getMCTileEntity().getBlockPos(), animName));
    }
}